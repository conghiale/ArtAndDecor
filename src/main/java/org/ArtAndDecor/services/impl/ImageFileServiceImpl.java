...
public class ImageFileServiceImpl implements ImageFileService {
    ...
    @Override
    public String uploadImage(MultipartFile file, String imageDisplayName) throws IOException {
        logger.info("Uploading image file: {}", imageDisplayName);
...
//        Hash file content with timestamp using Utils
        ...
        String hashedFilename = Utils.hashFileContentWithTimestamp(fileContent, fileExtension);
        Path filePath = storageDir.resolve(hashedFilename);

//        Save file
        try (InputStream inputStream = file.getInputStream()) {
            Files.copy(inputStream, filePath);
            logger.info("Image uploaded successfully: {} -> {}", imageDisplayName, hashedFilename);
        } catch (IOException e) {
            logger.error("Failed to upload image {}: {}", imageDisplayName, e.getMessage(), e);
            throw e;
        }
    ...
    @Override
    public String replaceImage(MultipartFile file, String imageDisplayName) throws IOException {
        logger.info("Replacing image file: {}", imageDisplayName);
...
//        Delete existing file if exists
        if (Files.exists(filePath)) {
            try {
                Files.delete(filePath);
                logger.debug("Deleted existing image file: {}", hashedFilename);
            } catch (IOException e) {
                logger.error("Failed to delete existing image file {}: {}", hashedFilename, e.getMessage());
            }
        }
...
//        Save new file
        try (InputStream inputStream = file.getInputStream()) {
            Files.copy(inputStream, filePath);
            logger.info("Image replaced successfully: {} -> {}", imageDisplayName, hashedFilename);
        } catch (IOException e) {
            logger.error("Failed to replace image {}: {}", imageDisplayName, e.getMessage(), e);
            throw e;
        }
