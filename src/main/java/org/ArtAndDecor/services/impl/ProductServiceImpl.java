...
public class ProductServiceImpl implements ProductService {
    ...
    public Page<ProductDto> getTopSellingProducts(Pageable pageable) {
        ...
        // Get all enabled products (should naturally sort by soldQuantity if repository query is updated)
        // For now, we use the same logic as getLatestProducts since getProductsByCriteria doesn't handle soldQuantity sorting
        Page<ProductDto> topSellingProducts = getProductsByCriteria(null, true, null, null, null, null, null, null, null, null, pageable);
        
        // If no enabled products found, log the issue
        if (topSellingProducts.getTotalElements() == 0) {
            logger.warn("No enabled products found in the system for top selling query");
        }
